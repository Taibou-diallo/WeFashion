     ...
     <x-app-layout>
         ...
         <x-slot name="header">
             <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                 {{ __('Liste de tous nos produits') }}
             </h2>
         </x-slot>
         <div class=" d-flex justify-content-between  mb-2 mt-4">
             {{-- pagination --}}
             {{ $products->links() }}
             <div> <a href="{{ route('product.create') }}" class="btn btn-primary">Nouveau</a></div>
         </div>
             ...
             <tbody>
                 @foreach ($products as $product)
                     <tr>
                         ...
                         <td> {{ $product->state }}</td>
                         <td>
                             <a href="{{ route('product.edit', $product) }}" class="btn btn-success">
